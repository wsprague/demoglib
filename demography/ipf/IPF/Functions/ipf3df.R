...
ipf3df <- function(rowcontrol, colcontrol, stackcontrol, seed=array(1,dim=c(length(rowcontrol),length(colcontrol),length(stackcontrol))), maxiter=50, closure=0.0001)
 {
    # input data checks: sum of marginal totals equal and no zeros in marginal totals
   if((sum(rowcontrol) != sum(colcontrol)) || (sum(rowcontrol) != sum(stackcontrol))) {
warning(paste("Row control=",sum(rowcontrol)," Colcontrol=",sum(colcontrol)," stackcontrol=",sum(stackcontrol)))
stop("Marginal control vectors must sum to the same grand total.")}
   ...
   if(any(stackcontrol==0)){
      ...
      warning(paste(numzero, "zeros in stackcontrol argument replaced with 0.001", sep=" "))
      }
   if(any(seed==0)){
      numzero <- sum(seed==0)
      seed[seed==0] <- 0.001
...
#are we done yet?
checksum <- max(sum(abs(1-rowfactor)),sum(abs(1-colfactor)),sum(abs(1-stackfactor)))
iter <- iter + 1
      }
...
#Get rid of any fuzzies that were added to zeros in the control.
   ...
   stackcontrol <- round(stackcontrol,digits=0)

#Now we round and compute the three adjustments, just to see where we are
   result <- round(result, digits=0)
   row.corn <- rowcontrol-apply(result,c(3,2),sum)
   col.corn <- colcontrol-apply(result,c(3,1),sum)
   stack.corn <- stackcontrol-apply(result,c(2,1),sum)

   result <- list(fitted.table=result, number.iterations=iter, tolerance=checksum, row.adj=row.corn, col.adj=col.corn,stack.adj=stack.corn)
   result
   }
